              ...
              <LiftOnHover>
                <AppLink className="block" href={`/work/${item.slug}`}>
                  ...
                  >
                    ...
                    <div className={`absolute bottom-8 left-8 flex items-center gap-3 rounded-full border px-4 py-2 text-[11px] font-label-mono uppercase tracking-[0.24em] backdrop-blur ${tone === 'light' ? 'border-white/25 bg-white/75 text-primary' : 'border-white/12 bg-primary-container/75 text-on-primary'}`}>
                  ...
                  <div className={`col-span-12 flex flex-col justify-center lg:col-span-5 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
                    <p className="surface-copy mb-4 font-label-mono text-label-mono">{String(index + 1).padStart(2, '0')}</p>
                    <h3 className="surface-title mb-6 font-headline-lg text-4xl">{item.title}</h3>
                    <p className="surface-copy mb-8 font-body-lg text-body-lg">{item.description}</p>
